...
//@todo Invalid query? check, 2 tables with ID so we need to specify which table we want.
$handle = $pdo->prepare('SELECT user.id, concat_ws(firstname, lastname, " ") AS name, sport
FROM user
    LEFT JOIN sport
